	...
	if (headers->sps.vuiParametersPresentFlag) {
		...
		if (headers->sps.nalHrdParametersPresentFlag) {
			...
			case 0:
				...
				headers->sps.naltimeOffsetLength = ((spsHeader[offset] & FTIMEOFFSETLENGT7H) << 4) | ((spsHeader[offset+1] & FTIMEOFFSETLENGTH7L) >> 4); //u(5)
				offset++;
				break;
			case 1:
			...
			case 5:
				...
				offset++;
				headers->sps.naltimeOffsetLength = ((spsHeader[offset] & FTIMEOFFSETLENGT4H) << 1) | ((spsHeader[offset+1] & FTIMEOFFSETLENGTH4L) >> 7); //u(5)
				offset++;
				break;
			case 6:
				...
				headers->sps.naltimeOffsetLength = ((spsHeader[offset] & FTIMEOFFSETLENGT5H) << 2) | ((spsHeader[offset+1] & FTIMEOFFSETLENGTH5L) >> 6); //u(5)
				offset++;
				break;
			case 7:
				...
				headers->sps.naltimeOffsetLength = ((spsHeader[offset] & FTIMEOFFSETLENGT6H) << 3) | ((spsHeader[offset+1] & FTIMEOFFSETLENGTH6L) >> 5); //u(5)
				offset++;
				break;
			}
		}
